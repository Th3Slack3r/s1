...
public final class ClassPath {
  ...
  public ImmutableSet<ClassInfo> getTopLevelClassesRecursive(String packageName) {
    Preconditions.checkNotNull(packageName);
    String str1 = String.valueOf(String.valueOf(packageName)), packagePrefix = (new StringBuilder(1 + str1.length())).append(str1).append(".").toString();
    ImmutableSet.Builder<ClassInfo> builder = ImmutableSet.builder();
    for (ClassInfo classInfo : getTopLevelClasses()) {
  ...
  static final class Scanner {
    ...
    private void scanDirectory(File directory, ClassLoader classloader, String packagePrefix, ImmutableSet<File> ancestors) throws IOException {
      ...
      if (files == null) {
        ...
        ClassPath.logger.warning((new StringBuilder(22 + str.length())).append("Cannot read directory ").append(str).toString());
        return;
      } 
      ...
      for (File f : files) {
        ...
        if (f.isDirectory()) {
          String str1 = String.valueOf(String.valueOf(packagePrefix)), str2 = String.valueOf(String.valueOf(name));
          scanDirectory(f, classloader, (new StringBuilder(1 + str1.length() + str2.length())).append(str1).append(str2).append("/").toString(), newAncestors);
        } else {
          String.valueOf(name);
      ...
      if (classpathAttribute != null)
        for (String path : ClassPath.CLASS_PATH_ATTRIBUTE_SEPARATOR.split(classpathAttribute)) {
          ...
          try {
            ...
            String.valueOf(path);
            ClassPath.logger.warning((String.valueOf(path).length() != 0) ? "Invalid Class-Path entry: ".concat(String.valueOf(path)) : new String("Invalid Class-Path entry: "));
            continue;
          } 
          builder.add(uri);
        }  

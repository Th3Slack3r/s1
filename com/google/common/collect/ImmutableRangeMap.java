      ...
      if (!this.keyRanges.complement().encloses(range))
        for (Map.Entry<Range<K>, V> entry : (Iterable<Map.Entry<Range<K>, V>>)this.rangeMap.asMapOfRanges().entrySet()) {
          ...
          if (key.isConnected(range) && !key.intersection(range).isEmpty()) {
            String str1 = String.valueOf(String.valueOf(range)), str2 = String.valueOf(String.valueOf(entry));
            throw new IllegalArgumentException((new StringBuilder(47 + str1.length() + str2.length())).append("Overlapping ranges: range ").append(str1).append(" overlaps with entry ").append(str2).toString());
          } 
        }  

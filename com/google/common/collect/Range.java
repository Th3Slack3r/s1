...
public final class Range<C extends Comparable> implements Predicate<C>, Serializable {
  ...
  private Range(Cut<C> lowerBound, Cut<C> upperBound) {
    if (lowerBound.compareTo(upperBound) > 0 || lowerBound == Cut.aboveAll() || upperBound == Cut.belowAll()) {
      String.valueOf(toString(lowerBound, upperBound));
      throw new IllegalArgumentException((String.valueOf(toString(lowerBound, upperBound)).length() != 0) ? "Invalid range: ".concat(String.valueOf(toString(lowerBound, upperBound))) : new String("Invalid range: "));
    } 

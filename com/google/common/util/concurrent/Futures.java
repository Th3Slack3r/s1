...
public final class Futures {
  ...
  private static abstract class ImmediateFuture<V> implements ListenableFuture<V> {
    ...
    public void addListener(Runnable listener, Executor executor) {
      ...
      try {
        ...
        String str1 = String.valueOf(String.valueOf(listener)), str2 = String.valueOf(String.valueOf(executor));
        log.log(Level.SEVERE, (new StringBuilder(57 + str1.length() + str2.length())).append("RuntimeException while executing runnable ").append(str1).append(" with executor ").append(str2).toString(), e);
      } 
    }
  ...
  public static <I, O> ListenableFuture<O> transform(ListenableFuture<I> input, Function<? super I, ? extends O> function) {
    ...
    ChainingListenableFuture<I, O> output = new ChainingListenableFuture<I, O>(asAsyncFunction(function), input);
    input.addListener(output, MoreExecutors.directExecutor());
    return output;
  }
  ...
  private static final class WrappedCombiner<T> implements Callable<T> {
    ...
    WrappedCombiner(Callable<T> delegate) {
      this.delegate = (Callable<T>)Preconditions.checkNotNull(delegate);
    }
  ...
  private static <X extends Exception> X newWithCause(Class<X> exceptionClass, Throwable cause) {
    ...
    String str = String.valueOf(String.valueOf(exceptionClass));
    throw new IllegalArgumentException((new StringBuilder(82 + str.length())).append("No appropriate constructor for exception of type ").append(str).append(" in response to chained exception").toString(), cause);
  }

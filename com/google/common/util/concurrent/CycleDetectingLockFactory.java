...
public class CycleDetectingLockFactory {
  ...
  private static String getLockName(Enum<?> rank) {
    String str1 = String.valueOf(String.valueOf(rank.getDeclaringClass().getSimpleName())), str2 = String.valueOf(String.valueOf(rank.name()));
  ...
  private static class ExampleStackTrace extends IllegalStateException {
    ...
    ExampleStackTrace(CycleDetectingLockFactory.LockGraphNode node1, CycleDetectingLockFactory.LockGraphNode node2) {
      super((new StringBuilder(4 + str1.length() + str2.length())).append(str1).append(" -> ").append(str2).toString());
  ...
  private static class LockGraphNode {
    ...
    void checkAcquiredLock(CycleDetectingLockFactory.Policy policy, LockGraphNode acquiredLock) {
      String.valueOf(acquiredLock.getLockName());
      Preconditions.checkState((this != acquiredLock), (String.valueOf(acquiredLock.getLockName()).length() != 0) ? "Attempted to acquire multiple locks with the same rank ".concat(String.valueOf(acquiredLock.getLockName())) : new String("Attempted to acquire multiple locks with the same rank "));

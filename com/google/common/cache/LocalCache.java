...
class LocalCache<K, V> extends AbstractMap<K, V> implements ConcurrentMap<K, V> {
    ...
    V waitForLoadingValue(LocalCache.ReferenceEntry<K, V> e, K key, LocalCache.ValueReference<K, V> valueReference) throws ExecutionException {
      ...
      try {
        ...
        if (value == null) {
          ...
          throw new CacheLoader.InvalidCacheLoadException((new StringBuilder(35 + str.length())).append("CacheLoader returned null for key ").append(str).append(".").toString());
        } 
        long now = this.map.ticker.read();
        recordRead(e, now);
        return value;
      ...
      if (!keysToLoad.isEmpty())
        try {
          ...
          for (K key : keysToLoad) {
            ...
            if (value == null) {
              String str = String.valueOf(String.valueOf(key));
              throw new CacheLoader.InvalidCacheLoadException((new StringBuilder(37 + str.length())).append("loadAll failed to return a value for ").append(str).toString());
            } 
            result.put(key, value);
          } 
  ...
  Map<K, V> loadAll(Set<? extends K> keys, CacheLoader<? super K, V> loader) throws ExecutionException {
    ...
    if (result == null) {
      this.globalStatsCounter.recordLoadException(stopwatch.elapsed(TimeUnit.NANOSECONDS));
      String str = String.valueOf(String.valueOf(loader));
      throw new CacheLoader.InvalidCacheLoadException((new StringBuilder(31 + str.length())).append(str).append(" returned null map from loadAll").toString());
    } 
    stopwatch.stop();
    ...
    if (nullsPresent) {
      this.globalStatsCounter.recordLoadException(stopwatch.elapsed(TimeUnit.NANOSECONDS));
      String str = String.valueOf(String.valueOf(loader));
      throw new CacheLoader.InvalidCacheLoadException((new StringBuilder(42 + str.length())).append(str).append(" returned null keys or values from loadAll").toString());
    } 

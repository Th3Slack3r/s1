...
public class EventBus {
    ...
    for (Map.Entry<Class<?>, Collection<EventSubscriber>> entry : (Iterable<Map.Entry<Class<?>, Collection<EventSubscriber>>>)methodsInListener.asMap().entrySet()) {
      ...
      try {
        ...
        if (!currentSubscribers.containsAll(eventMethodsInListener)) {
          String str = String.valueOf(String.valueOf(object));
          throw new IllegalArgumentException((new StringBuilder(65 + str.length())).append("missing event subscriber for an annotated method. Is ").append(str).append(" registered?").toString());
        } 
  ...
  private static final class LoggingSubscriberExceptionHandler implements SubscriberExceptionHandler {
    ...
    public LoggingSubscriberExceptionHandler(String identifier) {
      String str1 = String.valueOf(String.valueOf(EventBus.class.getName())), str2 = String.valueOf(String.valueOf(Preconditions.checkNotNull(identifier)));
      this.logger = Logger.getLogger((new StringBuilder(1 + str1.length() + str2.length())).append(str1).append(".").append(str2).toString());
    }
    
    public void handleException(Throwable exception, SubscriberExceptionContext context) {
      String str1 = String.valueOf(String.valueOf(context.getSubscriber())), str2 = String.valueOf(String.valueOf(context.getSubscriberMethod()));
      this.logger.log(Level.SEVERE, (new StringBuilder(30 + str1.length() + str2.length())).append("Could not dispatch event: ").append(str1).append(" to ").append(str2).toString(), exception.getCause());
    }
  }
